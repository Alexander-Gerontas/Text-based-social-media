package com.alg.social_media.post.adapter.out.persistance;
...
public class CommentRepositoryImpl extends BaseRepositoryImpl<Comment, Long> implements
  ...
  public List<Comment> findAccountPostCommentsReverseChronologically(Long accountId, int page,
    ...
    DBUtils.DbTransactionResultOperation<List<Comment>> operation = entityManager -> {
      ...
      Predicate accountCondition = cb.equal(commentRoot.get("post").get("author").get("id"),
          accountId);
  ...
  @Override
  public List<Comment> findFollowersPostCommentsReverseChronologically(List<Long> accountIds,
      int page,
      int commentLimit) {
    DBUtils.DbTransactionResultOperation<List<Comment>> operation = entityManager -> {
      CriteriaBuilder cb = entityManager.getCriteriaBuilder();
